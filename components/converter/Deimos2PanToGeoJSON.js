...
const getDeimos2PanGeoJSON = (Deimos2PanObject) => {
    try {
            const root = Deimos2PanObject.Dimap_Document;
            ...
            root.Dataset_Sources[0].Source_Information[0].Quality_Assessment[0].Quality_Parameter.forEach(tag => {
              ...
              switch (parameterDesc) {
                case "ADJUSTMENT":
                  qualityParameters["ADJUSTMENT"] = parameterValue; // Convert to integer, set to -1 if not a valid number
                  break;
                ...
                case "ORBIT_DIRECTION":
                  qualityParameters["ORBIT_DIRECTION"] = parameterValue; // Convert to integer, set to -1 if not a valid number
                  break;
                ...
                case "FOOTPRINT_CENTER_OF":
                  qualityParameters["FOOTPRINT_CENTER_LAT"] = parseFloat(parameterValue.__value__.substring(6, parameterValue.__value__.length - 1).split(";")[1]) || 0;
                  qualityParameters["FOOTPRINT_CENTER_LON"] = parseFloat(parameterValue.__value__.substring(6, parameterValue.__value__.length - 1).split(";")[0]) || 0;
                  break;
          ...
          "features": [
            {
              "type": "Feature",
              "properties": {
                "BAND_DESCRIPTION": root.Image_Interpretation?.[0].Spectral_Band_Info?.[0].BAND_DESCRIPTION?.[0] ?? "NO_DATA",
                "BAND_INDEX": parseInt(root.Image_Interpretation?.[0].Spectral_Band_Info?.[0].BAND_INDEX?.[0] ?? -1),
                ...
                "MISSION_INDEX": parseInt(root.Dataset_Sources?.[0].Source_Information?.[0].Scene_Source?.[0].MISSION_INDEX?.[0] ?? -1),
        ...
        console.log(`${new Date().toISOString()}: Deimos2PanToGeoJSON - Info Log - ${Deimos2PanObject.custom.cstm_file_name} sucessfully converted.`);
        logger(`${new Date().toISOString()}: Deimos2PanToGeoJSON - Info Log - ${Deimos2PanObject.custom.cstm_file_name} sucessfully converted.`);
        
        return deimos2GeoJson;

    } catch(err) {
      console.log(`${new Date().toISOString()}: Deimos2PanToGeoJSON - Error Log - ${Deimos2PanObject.custom.cstm_file_name} converting error. - ${err}`);
      logger(`${new Date().toISOString()}: Deimos2PanToGeoJSON - Error Log - ${Deimos2PanObject.custom.cstm_file_name} converting error. - ${err}`);
        
      return {}
    }
}

export default getDeimos2PanGeoJSON;
export {setLogger};