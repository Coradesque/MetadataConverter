...
const getCSKGeoJSON = (CSKObject) => {
    try{
      
      const root = CSKObject.DeliveryNote;
      ...
      console.log(`${new Date().toISOString()}: CSKToGeoJSON - Info Log - ${CSKObject.custom.cstm_file_name} sucessfully converted.`);
      logger(`${new Date().toISOString()}: CSKToGeoJSON - Info Log - ${CSKObject.custom.cstm_file_name} sucessfully converted.`);
      
      return CSKGeoJSON;
      
    } catch (err) {
      console.log(`${new Date().toISOString()}: CSKToGeoJSON - Error Log - ${CSKObject.custom.cstm_file_name} converting error. - ${err}`);
      logger(`${new Date().toISOString()}: CSKToGeoJSON - Error Log - ${CSKObject.custom.cstm_file_name} converting error. - ${err}`);
      return {}
    }
  
  }
      
export default getCSKGeoJSON;
export {setLogger};